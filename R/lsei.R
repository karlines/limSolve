...
lsei <- function(A=NULL, B=NULL, E=NULL, F=NULL, G=NULL, H=NULL,
  ...
  if (type == 1)     {    # use LINPACKs lsei
    ...
    if (fulloutput)          # KEY = 1
      lpr <- lpr+3
    if (! is.null(tolrank))  # KEY 4, 5
      lpr <- lpr + 6
    if (! is.null(Wx))   {   # KEY 2, 3
      lw  <- length (Wx)
      lpr <- lpr + 2 + lw
    }
    ...
    if (lpr>1)   {
      ...
      if (! is.null(tolrank))   {
        if (length(tolrank) == 1)
          tolrank <- rep(tolrank, length.out = 2)
          ProgOpt[ipr:(ipr+5)] <- c(ipr+3, 4, tolrank[1],  # KS 23/6/2026 was: ipr+6
                                    ipr+6, 5, tolrank[2])
          ipr <- ipr+6
      }
      if (! is.null(Wx))    {
        ...
        if (lw == 1) {
          ...
          if (lw != Nx)
            stop("cannot solve least squares problem ", 
                 "- number of weighs should be =1 or =number of unknowns")
          lw <- lw + ipr + 1
    ...
    # KS 23/6/2026 was: added next 3 lines
    IP    <- rep(0, mIP)
    IP[1] <- mWS
    IP[2] <- mIP
    ...
    sol <-.Fortran("lsei",NUnknowns=Nx,NEquations=Neq,NConstraints=Nin,
             ...
             IP = as.integer(IP),  # KS 23/6/2026 was: as.integer(rep(0, mIP))
