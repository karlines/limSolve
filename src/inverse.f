...
C KARLINE: REMOVED roll from xDCOPY         
...
C     USES UNROLLED LOOPS FOR INCREMENTS EQUAL TO ONE.
C     JACK DONGARRA, LINPACK, 3/11/78.
C karline: simplified the subroutine - removed unrolled loops
...
C
C        CLEAN-UP LOOP
C karline: this part added
   20 DO I = 1, N
        DY(I) = DX(I)
      END DO
! karline: this part removed      
!   20 M = MOD(N,7)
!      IF( M .EQ. 0 ) GO TO 40
!      DO 30 I = 1,M
!        DY(I) = DX(I)
...
C KARLINE: CHANGED W(MDW,*) INTO W(MDW, N+1) ????
      ...
      DOUBLE PRECISION PRGOPT(*),RNORME,RNORML,W(MDW,N+1),WS(*),X(*)
c
